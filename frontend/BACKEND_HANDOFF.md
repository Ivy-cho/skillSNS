...
- **오프닝 턴 자리표시자 `(대화 시작)`를 이력에서 빼주세요** — 오프닝 턴을 만들 때
  ...
  메시지가 `user: "(대화 시작)"`로 와서, 사용자가 직접 친 말처럼 보입니다.
  **지금은 프론트에서 맨 앞 한 개만 걸러내고 있습니다**(`backendClient.getLatestChatSession`).
  임시방편이라 문구가 바뀌면 다시 새고, 나중에 다른 화면에서 기록을 쓸 때도 또 걸러야 합니다.
  `/latest`(그리고 기록을 내려주는 다른 엔드포인트)에서 **오프닝 자리표시자를 응답에 빼주시면**
  프론트 필터를 걷어내겠습니다. 세션에 오프닝 여부 플래그를 두고 첫 human 메시지를 스킵하는
  식이면 문구에 의존하지 않아 안전할 것 같습니다.
- **오프닝 턴만 있는 빈 대화가 세션으로 남습니다** — 피드·내 스킬·스크랩에서 스킬을 누르면
  지난 대화를 잇지 않고 **새 대화**로 열도록 바꿨습니다(`?new=1`). 그런데 오프닝 턴을 띄우려면
  `POST /chat/{skill_id}`를 불러야 하고, 그게 `ChatSession` 행을 먼저 만들어버립니다.
  그래서 **잠깐 눌러보고 나가기만 해도 채팅 목록에 빈 대화가 한 줄 생깁니다.**
  지금은 프론트에서 "직전 대화가 오프닝뿐이면 그걸 재사용"하는 식으로 무한정 쌓이는 것만
  막아뒀습니다(`SkillUsageChat` — `untouched`). 스킬당 최대 한 줄은 여전히 남습니다.
  둘 중 하나면 깔끔해집니다:
  1. 세션을 만들지 않고 오프닝 문구만 돌려주는 경로(예: `GET /chat/{skill_id}/opening`) —
     사용자가 실제로 첫 메시지를 보낼 때 그때 세션을 만든다, 또는
  2. `GET /chat/sessions`에서 **사용자 발화가 하나도 없는 세션은 빼고** 내려주기.
